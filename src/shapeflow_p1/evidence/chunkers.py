...
def markdown_structure_v1(
    ...
    is explicit. A table row chunk spans the header + separator + that row, so a selected row
    is never orphaned from the columns that give it meaning. Over-long blocks fall back to
    paragraph/sentence splitting while keeping their heading breadcrumb.
    """
    ...
    def emit_block(start: int, end: int, kind: str) -> None:
        if end <= start:
            return
        block = text[start:end]
        if tokenizer.count(block) <= max_tokens:
            chunks.append(_finalize(text, start, end, kind, tokenizer, breadcrumb()))
        else:
            for sub in paragraph_sentence_v1(block, tokenizer=tokenizer, max_tokens=max_tokens):
                chunks.append(
                    _finalize(text, start + sub.char_start, start + sub.char_end,
                              kind, tokenizer, breadcrumb())
                )
    ...
    while i < len(lines):
        ...
        if m:
            ...
            chunks.append(_finalize(text, line.start, line.end, "heading", tokenizer, breadcrumb()))
            i += 1
            continue
        ...
        if fence:
            ...
            chunks.append(_finalize(text, start, end, "code", tokenizer, breadcrumb()))
            i = j + 1
            continue
        ...
        if "|" in line.text and i + 1 < len(lines) and _TABLE_SEP.match(lines[i + 1].text):
            ...
            while j < len(lines) and "|" in lines[j].text and lines[j].text.strip():
                ...
                j += 1
            # Emit the header itself once, then one chunk per row carrying the header+sep.
            chunks.append(
                _finalize(text, header.start, sep.end, "table_header", tokenizer, breadcrumb())
            )
            header_prefix = text[header.start : sep.nl_end]
            for row in row_lines:
                # The chunk's span is the row; its text carries the header for context via a
                # synthesized reconstruction is NOT allowed (must be exact), so we anchor the
                # chunk on the contiguous header..row region to keep exact reconstruction.
                chunks.append(
                    _finalize(text, header.start, row.end, "table_row", tokenizer, breadcrumb())
                )
            i = j
            continue
